...
export const guardarPermisosRol = async (idRol, permisos) => {
  const response = await axios.post(`${API_URL}/rol-permisos/guardar_permisos/`, {
    id_rol: idRol,
...
export const getPermisosPorUsuario = async (idUsuario) => {
  const response = await axios.get(`${API_URL}/usuario-permisos/?usuario=${idUsuario}`);
  return response.data;
};

// Busca un usuario por número de documento
export const buscarUsuarioPorDocumento = async (numeroDocumento) => {
  const response = await axios.get(`${API_URL}/usuarios/?documento=${numeroDocumento}`);
  return response.data;
};

// Guarda todos los permisos de un usuario de una sola vez
export const guardarPermisosUsuario = async (idUsuario, permisos) => {
  const response = await axios.post(`${API_URL}/usuario-permisos/guardar_permisos/`, {
...
export const getPermisosCombinados = async (idUsuario) => {
  const response = await axios.get(`${API_URL}/usuarios/${idUsuario}/permisos-combinados`);
  return response.data;
};