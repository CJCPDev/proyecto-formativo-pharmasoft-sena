...
import { getPermisosPorUsuario, getPermisosCombinados } from "../services/permisosService";
import { getRoles } from "../services/usuarioService";

export default function PermisosModal({ isOpen, onClose, onSave, userId, userGroupId }) {
    ...
    const cargarPermisos = async () => {
      try {
        ...
        if (userId) {
          ...
          const { getPermisosPorRol } = await import("../services/permisosService");
  ...
  const handleSaveClick = () => {
    ...
    permissionGroups.forEach((group) => {
      group.permissions.forEach((permission) => {
        ...
        if (selectedPermissions[permission.id] && !permisosDelRol[permission.id]) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col">
                  ...
                  {group.permissions.map((permission) => (
                    ...
                    >
                      ...
                      <span className={`text-sm ${
                        selectedPermissions[permission.id] && !permisosDelRol[permission.id]
                          ? 'text-blue-600 font-medium'
                          : ''
                      }`}>
                        {permission.label}
                        {selectedPermissions[permission.id] && !permisosDelRol[permission.id] && (
                          <span className="ml-1 text-xs text-blue-400">(extra)</span>
                        )}
                      </span>
                    </div>
                  ))}
          ...
          <div className="flex justify-end gap-4 p-6 border-t">
            ...
            <Button variant="primary" size="md" onClick={handleSaveClick} disabled={loading}>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm mx-4 flex flex-col gap-4">
            ...
            <div className="flex justify-center gap-4">
              <Button variant="secondary" size="sm" onClick={() => setShowConfirmacion(false)}>
                Cancelar
              </Button>
