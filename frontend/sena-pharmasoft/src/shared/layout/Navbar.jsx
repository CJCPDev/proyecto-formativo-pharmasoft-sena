...
// Navbar.jsx
// Barra de navegación principal
// Muestra opciones según el rol y permisos del usuario
// ─────────────────────────────────────────────
...
const Navbar = ({ variant = "solid" }) => {
  ...
  const {
    puedeListarUsuarios,
    puedeListarProveedores,
    puedeListarMedicamentos,
    puedeListarVentas,
    puedeListarCarritos,
  } = usePermisos();
  ...
  const NavLinksDesktop = () => (
    <>
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarUsuarios)) && (
        <li>
          <Link to="/usuarios"
            className="hover:text-primary transition hover:underline hover:underline-offset-2">
            Usuarios
      ...
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarProveedores)) && (
        <li>
          <Link to="/listar-proveedor"
            className="hover:text-primary transition hover:underline hover:underline-offset-2">
            Proveedores
          </Link>
        </li>
      )}
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarMedicamentos)) && (
        <li>
      ...
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarVentas)) && (
        <li>
          <Link to="/listar-ventas"
            className="hover:text-primary transition hover:underline hover:underline-offset-2">
            Ventas
          </Link>
        </li>
      )}
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarCarritos)) && (
        <li>
          <Link to="/carritos"
            className="hover:text-primary transition hover:underline hover:underline-offset-2">
            Carritos
  ...
  const NavLinksMobile = () => (
    <>
      ...
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarProveedores)) && (
        <li>
          <Link to="/listar-proveedor" onClick={() => setIsMenuOpen(false)}
            className="block px-4 py-2 hover:bg-brand-hover/20 transition rounded-2xl hover:underline hover:underline-offset-2">
            Proveedores
          </Link>
        </li>
      ...
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarMedicamentos)) && (
        <li>
          <Link to="/medicamentos" onClick={() => setIsMenuOpen(false)}
            className="block px-4 py-2 hover:bg-brand-hover/20 transition rounded-2xl hover:underline hover:underline-offset-2">
            Medicamentos
          </Link>
        </li>
      ...
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarVentas)) && (
        <li>
          <Link to="/listar-ventas" onClick={() => setIsMenuOpen(false)}
            className="block px-4 py-2 hover:bg-brand-hover/20 transition rounded-2xl hover:underline hover:underline-offset-2">
            Ventas
          </Link>
        </li>
      ...
      {(ES_ADMIN || (ES_FARMACEUTA && puedeListarCarritos)) && (
        <li>
          <Link to="/carritos" onClick={() => setIsMenuOpen(false)}
            className="block px-4 py-2 hover:bg-brand-hover/20 transition rounded-2xl hover:underline hover:underline-offset-2">
            Carritos
          </Link>
        </li>
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative md:hidden">
                ...
                {isMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-brand-soft/40 backdrop-blur-md shadow-xl rounded-2xl">
                    <ul className="text-sm text-brand-hover font-bold">
                      <NavLinksMobile/>
                    </ul>
                  </div>
                )}
